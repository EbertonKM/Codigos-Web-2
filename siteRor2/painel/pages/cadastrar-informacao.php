...
<div class="box-content">
	<h2><i class="fa-regular fa-newspaper"></i> Adicionar Informação</h2>
		...
		if(isset($_POST['acao'])) {
			...
			if($titulo == '' || $conteudo == '') {
				...
				if(Painel::validImage($capa) == false) {
					...
					$verificarNoticia = MySql::conectar()->prepare("SELECT * FROM `$tabela` WHERE titulo = ?");
					$verificarNoticia->execute(array($titulo));
