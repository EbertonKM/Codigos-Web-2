...
<div class="box-content">
	<h2><i class="fa-solid fa-user-pen"></i> Editar Informação</h2>
	...
	<form method="post" enctype="multipart/form-data">
		...
		if(isset($_POST['acao'])) {
			...
			$verificarNoticia = MySql::conectar()->prepare("SELECT * FROM `$tabela` WHERE titulo = ? AND id != ?");
			$verificarNoticia->execute(array($titulo, $id));
			if($verificarNoticia->rowCount() == 0) {
				...
				//O usuário selecionou uma imagem
					if(Painel::validImage($imagem)) {
						...
						$arr = ['titulo' => $titulo, 'conteudo' => $conteudo, 'capa' => $imagem, 'slug' => $slug];
